...
int click_newfilebutton(buttonmenu_t *button)
{
...
int hover_newfilebutton(buttonmenu_t *button)
{
    ...
    mouse_pos.y <= button_pos.y + button_size.y) {
        ...
        return (click_newfilebutton(button));
    } else {
...
buttonmenu_t *set_newfilebutton(buttonmenu_t *button)
{
    ...
    sfText_setString(button->text, "Newfile");
...
buttonmenu_t *init_newfilebutton(void)
{
    ...
    button->position = (sfVector2f){0, 50};
    ...
    button->text_pos = (sfVector2f){button->position.x + 55,
    button->position.y + 6};
    button->text_color = sfBlack;
    button->text_size = 30;

    button = set_newfilebutton(button);
